...
pub const HashMap = struct {
    ...
    pub inline fn keys(self: @This()) []String {
        return self._keys[0..self.cap];
    }
    pub inline fn values(self: @This()) []String {
        return self._values[0..self.cap];
    }
    pub inline fn meta(self: @This()) []u8 {
        return self._meta[0..self.cap];
    }
    ...
    pub fn init(keys_string: []const u8, cap: Size, allocator: std.mem.Allocator) !@This() {
        ...
        return .{
            ...
            ._keys = @ptrCast(@alignCast(mem.ptr)),
            ._values = @ptrCast(@alignCast(mem[c * @sizeOf(String) ..].ptr)),
    ...
    fn allocation(self: *@This()) []align(@alignOf(String)) u8 {
        return @as([*]align(@alignOf(String)) u8, @ptrCast(@alignCast(self._keys)))[0 .. (2 * @sizeOf(String) + 1) * self.cap];
    }
...
pub const ComptimeEnvType = struct {
    ...
    pub const Iterator = struct {
        ...
        pub fn next(it: *Iterator) ?KV {
            ...
            while (it.i < it.map.capacity()) {
                ...
                return .{ .key = it.map.data[@intCast(bucket.key_idx)..][0..@intCast(bucket.key_len)], .value = it.map.data[0..@intCast(next_bucket.key_idx)][@intCast(bucket.key_idx)..][@intCast(bucket.key_len)..] };
            }
            return null;
        }
    };
    ...
    pub inline fn buckets(self: @This()) []const Bucket {
        return self._buckets[0 .. self.cap + 1];
    }
    pub inline fn meta(self: @This()) []const u8 {
        return self._meta[0..self.cap];
    }
...
pub const EnvType = struct {
    ...
    pub const Iterator = struct {
        ...
        pub fn next(it: *Iterator) ?KV {
            ...
            while (it.i < it.cap) {
                ...
                return .{ .key = it.data[@intCast(bucket.key_idx)..][0..@intCast(bucket.key_len)], .value = it.data[0..@intCast(next_bucket.key_idx)][@intCast(bucket.key_idx)..][@intCast(bucket.key_len)..] };
            }
            return null;
        }
    };
    ...
    pub inline fn data(self: @This()) []const u8 {
        return self._meta[self.cap..][0..self._data_size];
    }
    pub inline fn count(self: @This()) usize {
        return self.size;
    }
                    ...
                    switch (if (quote_char) |c| @as(u9, c) else 0x100) { // switch on the quote_char
                        ...
                        '"' => switch (self.takeU9()) {
                            ...
                            inline '$', '\"' => |c| try self.map.values_string.append(self.map.allocator, c),
                                ...
                                options.log_fn("Unexpected escape sequence `\\{s}` in a{s} value at ", .{ escaped(@intCast(c_u9)) orelse self.currentAsSlice(), quote_string });
                                self.at -= 1;
                                self.printErrorMarker();
                                return ParseValueError.InvalidEscapeSequence;
                            },
